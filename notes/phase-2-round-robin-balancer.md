...
- **No live backend membership changes.** The backend list is fixed at process start by `-backend` flags and never changes; there's no admin-triggered add/remove while the process is running. Raised in PR review (see PR #2) and deliberately deferred: this is mechanistically the same problem health checking needs to solve (marking a backend down is "remove from rotation," recovery is "add back"), including the concurrency question of swapping the backend set safely under every in-flight request's concurrent `Next()` calls (likely an `atomic.Pointer` to an immutable slice rather than a mutable structure like a linked list). Solving it once, for health checks, rather than building an ad hoc admin-command version first.
