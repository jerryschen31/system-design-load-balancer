# Phase 4a Test Report: Backend timeouts and a concurrency limiter (no queue)

**Source:** uncommitted work on `phase-4a-timeouts-backpressure`, branched from `build` at `e7b55f5` (tip: "added /hang endpoint to echobackend for mock testing hanging backends")
**Command:** `go test -v -race -count=1 ./cmd/... ./internal/...`
**Result:** 51 passed, 0 failed, across 6 packages

| Package | Result | Duration |
|---|---|---|
| `cmd/echobackend` | ok | 1.27s |
| `cmd/loadbalancer` | ok | 1.69s |
| `internal/balancer` | ok | 1.68s |
| `internal/healthcheck` | ok | 2.59s |
| `internal/middleware` | ok | 1.78s |
| `internal/proxy` | ok | 2.98s |

`./...` is scoped to `./cmd/... ./internal/...` here rather than the whole module: the repo also contains `scratch/`, a pair of standalone exploratory files (`understand_context.go`, `understand_context_and_signal.go`) that both declare `package main` with conflicting `func main`/`runWorker` — this predates phase 4a (confirmed via `git stash`) and isn't part of the module's real build.

Generated on 2026-08-24 directly from the phase-4a branch. See `notes/phase-4a-timeouts-backpressure.md` for the narrative writeup this output supports.
...
PASS
ok  	github.com/jerryschen31/system-design-load-balancer/cmd/echobackend	1.265s
=== RUN   TestIntegration_ChecksRouteAroundUnhealthyBackend
    integration_test.go:102: input: 2 backends, one (http://127.0.0.1:56488) already unhealthy before the checker ever runs
...
=== RUN   TestStress_RecoveredBackendImmediatelyGetsFullShare
    integration_test.go:151: input: 3 backends; backend 0 (http://127.0.0.1:56542) starts unhealthy, 1 and 2 start healthy
...
--- PASS: TestStress_RecoveredBackendImmediatelyGetsFullShare (0.15s)
...
--- PASS: TestStress_ConcurrentTrafficSurvivesHealthFlapping (0.04s)
...
PASS
ok  	github.com/jerryschen31/system-design-load-balancer/cmd/loadbalancer	1.691s
...
--- PASS: TestRoundRobinConcurrentSetHealthyAndNext (0.18s)
...
PASS
ok  	github.com/jerryschen31/system-design-load-balancer/internal/balancer	1.676s
=== RUN   TestCheckerDetectsHealthyBackend
    healthcheck_test.go:48: input: backend at http://127.0.0.1:56721 answers /health with 200
...
=== RUN   TestCheckerDetectsUnhealthyStatus
    healthcheck_test.go:71: input: backend at http://127.0.0.1:56730 answers /health with 500
...
=== RUN   TestCheckerDetectsTimeout
    healthcheck_test.go:98: input: backend at http://127.0.0.1:56744 sleeps 500ms before answering; checker timeout is 50ms
...
PASS
ok  	github.com/jerryschen31/system-design-load-balancer/internal/healthcheck	2.591s
=== RUN   TestMaxConcurrentAllowsExactlyNInFlight
    concurrency_test.go:28: input: MaxConcurrent(3) wrapping a handler that blocks until released
    concurrency_test.go:54: step: 3 requests confirmed in flight (holding every semaphore slot)
    concurrency_test.go:78: step: 2 additional requests while at capacity -> statuses [503 503]
    concurrency_test.go:87: output: the 3 originally-admitted requests all completed with statuses [200 200 200]
    concurrency_test.go:112: output: after release, a fresh batch of n requests all succeeded -- slots are correctly reusable
--- PASS: TestMaxConcurrentAllowsExactlyNInFlight (0.00s)
=== RUN   TestMaxConcurrentPanicsOnNonPositiveN
    concurrency_test.go:116: input: MaxConcurrent(0)
    concurrency_test.go:122: output: panicked as expected: middleware: MaxConcurrent requires a positive n
...
PASS
ok  	github.com/jerryschen31/system-design-load-balancer/internal/middleware	1.777s
=== RUN   TestForwardsRequestToBackend
    proxy_test.go:62: input: client sends POST /widgets to load balancer http://127.0.0.1:56916; backend is http://127.0.0.1:56915
...
=== RUN   TestClientCannotSpoofForwardedFor
    proxy_test.go:100: input: client sends X-Forwarded-For="6.6.6.6" to http://127.0.0.1:56920
...
=== RUN   TestBackendDownReturns502
    proxy_test.go:157: input: backend http://127.0.0.1:1 is down; client sends GET / through load balancer http://127.0.0.1:56927
...
=== RUN   TestRewriteLogsRoutingDecision
    proxy_test.go:343: input: GET / through the load balancer, backend is http://127.0.0.1:56947
    proxy_test.go:352: output: logged "routed GET / -> http://127.0.0.1:56947\n"
...
=== RUN   TestStress_DisabledTimeoutWaitsIndefinitely
    ...
    stress_test.go:44: output: client returned after 51.414417ms with error Get "http://127.0.0.1:56952/": context deadline exceeded (Client.Timeout exceeded while awaiting headers) -- opt-out confirmed working
...
=== RUN   TestStress_ConcurrentRequestsHandledConcurrently
    ...
    stress_test.go:96: output: burst finished in 117.662ms (serial time would have been 5s)
...
PASS
ok  	github.com/jerryschen31/system-design-load-balancer/internal/proxy	2.981s
```
